...
using Xocv.Authorization.Impersonation;
using Xocv.Authorization.Roles;
using Xocv.Authorization.Users;
using Xocv.Configuration;
using Xocv.Debugging;
using Xocv.MultiTenancy;
using Xocv.Notifications;
using Xocv.Web.Controllers.Results;
using Xocv.Web.Models.Account;
using Xocv.Web.MultiTenancy;
using Recaptcha.Web;
...
{
    public class AccountController : XocvControllerBase
    {
        ...
        public async Task<JsonResult> VerifySecurityCode(VerifySecurityCodeViewModel model)
        {
            ...
            using (CurrentUnitOfWork.SetTenantId(tenantId))
            {
                var result = await _signInManager.TwoFactorSignInAsync( //TODO: Override from Abp.Zero?
                    model.Provider,
                    model.Code,
