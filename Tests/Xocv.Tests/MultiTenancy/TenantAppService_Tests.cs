...
using Xocv.MultiTenancy.Dto;
using Xocv.Notifications;
using Shouldly;
using Xunit;

namespace Xocv.Tests.MultiTenancy
{
        ...
        public async Task Create_Update_And_Delete_Tenant_Test()
        {
            ...
            await _tenantAppService.CreateTenant(
                new CreateTenantInput
                {
                    ...
                    IsActive = true,
                    ConnectionString = "Server=localhost; Database=XocvTest_" + Guid.NewGuid().ToString("N") + "; Trusted_Connection=True;"
                });
