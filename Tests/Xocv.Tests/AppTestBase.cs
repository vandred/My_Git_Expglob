...
using Xocv.Migrations.Seed.Host;
using Xocv.Migrations.Seed.Tenants;
using Xocv.MultiTenancy;
using Xocv.Tests.TestDatas;

namespace Xocv.Tests
{
    /// <summary>
    /// This is base class for all our test classes.
    /// It prepares ABP system, modules and a fake, in-memory database.
    /// Seeds database with initial data.
    /// Provides methods to easily work with <see cref="XocvDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<XocvTestModule>
    {
        ...
        protected void UsingDbContext(Action<XocvDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<XocvDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<XocvDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<XocvDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<XocvDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<XocvDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<XocvDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<XocvDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<XocvDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<XocvDbContext, Task<T>> func)
        {
            T result;
